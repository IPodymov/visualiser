...
import { ComparePage } from '@features/comparison/ui/ComparePage';
...
vi.mock('@features/comparison/api/comparison', () => ({
  comparisonApi: { compare: vi.fn() },
}));
vi.mock('@features/comparison/ui/PlanSelector/PlanSelector', async () => {
  ...
  return {
    PlanSelector: ({
      ...
      React.createElement(
        'div',
        { 'data-testid': `selector-${side}` },
        React.createElement('span', null, `${side}:${value ?? 'empty'}:${requiredLevel ?? 'any'}`),
        React.createElement(
          'button',
          { type: 'button', onClick: () => onChange(side === 'A' ? 1 : 2) },
          `select-${side}`,
        ),
        React.createElement(
          'button',
        ...
        React.createElement(
          'button',
          { type: 'button', onClick: () => onChange(999) },
          `invalid-${side}`,
        ),
      ),
  };
...
const richComparison: PlanComparison = {
  ...
  commonDisciplines: [
    {
      name: 'Общая',
      first: first.disciplines[0],
      second: second.disciplines[0],
      differences: [{ field: 'totalHours', firstValue: 200, secondValue: 150 }],
    },
    {
      name: 'Совпадающая',
      first: first.disciplines[0],
...
const emptyComparison: PlanComparison = {
  ...
  summary: {
    firstDisciplinesCount: 0,
    secondDisciplinesCount: 0,
    commonCount: 0,
    onlyFirstCount: 0,
    onlySecondCount: 0,
  },
...
beforeEach(() => {
  ...
  vi.mocked(comparisonApi.compare).mockReset();
  vi.mocked(comparisonApi.compare).mockResolvedValue(richComparison);
...
describe('comparison user journey', () => {
  ...
  it('blocks incompatible or incomplete persisted selections before the API', async () => {
    ...
    const { rerender } = renderPage();
    expect(
      await screen.findByText('Для сравнения выберите две программы одного уровня образования.'),
    ).toBeInTheDocument();
    expect(comparisonApi.compare).not.toHaveBeenCalled();
  ...
  it('shows comparison loading, Error failures, fallback failures and retries', async () => {
    ...
    vi.mocked(comparisonApi.compare).mockImplementationOnce(
      () =>
    ...
    vi.mocked(comparisonApi.compare).mockRejectedValueOnce('failure');
    await userEvent.click(screen.getByRole('button', { name: 'Повторить' }));
    expect(
      await screen.findByText('Не удалось сравнить программы', { selector: 'p' }),
    ).toBeInTheDocument();
  });
  ...
  it('explains absent analytical evidence and empty discipline intersections', async () => {
    vi.mocked(comparisonApi.compare).mockResolvedValue(emptyComparison);
    useAppStore.setState({ compareIds: [1, 2], compareLevels: ['Бакалавриат', 'Бакалавриат'] });
    renderPage();
  ...
  it('describes smaller A totals in metric deltas', async () => {
    const reversed: PlanComparison = {
      ...
      firstPlan: plan({
        id: 1,
        title: 'Малая A',
        disciplines: [{ ...plan().disciplines[0], hours: 10, credits: 1 }],
      }),
      secondPlan: plan({
        id: 2,
        title: 'Большая B',
        disciplines: [{ ...plan().disciplines[0], hours: 20, credits: 2 }],
      }),
    };
  ...
  it('builds semester evidence when only program B has a numbered semester', async () => {
    const onlySecondSemester: PlanComparison = {
      ...
      firstPlan: plan({
        id: 1,
        disciplines: [{ ...plan().disciplines[0], semester: null, hours: 0 }],
      }),
      secondPlan: plan({
        id: 2,
        disciplines: [{ ...plan().disciplines[0], semester: 1, hours: 100 }],
      }),
    };
    vi.mocked(comparisonApi.compare).mockResolvedValue(onlySecondSemester);
    useAppStore.setState({ compareIds: [1, 2], compareLevels: ['Бакалавриат', 'Бакалавриат'] });
    renderPage();
    expect(
