...
describe('API client security and diagnostics', () => {
  it('requires an explicit API origin in production mode', async () => {
    ...
    await expect(import('@shared/api/client')).rejects.toThrow(
      'VITE_API_BASE_URL is required in production',
    );

    vi.resetModules();
    vi.stubEnv('VITE_API_BASE_URL', 'https://api.example.test');
    await expect(import('@shared/api/client')).resolves.toBeDefined();
  });
  ...
  it('adds bearer credentials only when a token exists', async () => {
    ...
    const handler = (
      client.interceptors.request as unknown as {
        handlers: Array<InterceptorHandler<RequestConfig>>;
      }
    ).handlers[0];
  ...
  it('passes successful responses through unchanged', async () => {
    ...
    const handler = (
      client.interceptors.response as unknown as {
        handlers: Array<InterceptorHandler<Record<string, unknown>>>;
      }
    ).handlers[0];
