...
export const DisciplineTable = ({ disciplines }: { disciplines: Discipline[] }) => (
  <>
          ...
          {disciplines.map((discipline) => (
            <TableRow key={`${discipline.id}-${discipline.name}-${discipline.semester ?? 0}`}>
              ...
              <TableCell className="text-right text-muted-foreground">
                {formatValue(discipline.lectureHours)} / {formatValue(discipline.practiceHours)} /{' '}
                {formatValue(discipline.labHours)}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
    ...
    <div className="grid gap-3 md:hidden">
      {disciplines.map((discipline) => (
        <article
          key={`${discipline.id}-${discipline.name}-${discipline.semester ?? 0}`}
        ...
        >
          ...
          <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <dd className="mt-1 font-medium">{discipline.semester ?? '—'}</dd>
            </div>
            <div>
              <dt className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <dd className="mt-1 font-medium">{discipline.controlForm || 'Не указано'}</dd>
            </div>
            <div>
              <dt className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <dd className="mt-1 font-medium">{discipline.hours || '—'} ч.</dd>
            </div>
            <div>
              <dt className="flex items-center gap-1.5 text-xs text-muted-foreground">
              ...
              <dd className="mt-1 font-medium">{discipline.credits || '—'}</dd>
            </div>
          </dl>
        </article>
      ))}
